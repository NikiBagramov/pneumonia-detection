...
model.fc = nn.Linear(num_ftrs, len(class_names))  # 2 класса: NORMAL и PNEUMONIA
...
def train_model(model, criterion, optimizer, num_epochs=10):
    ...
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.title('Loss over Epochs (ResNet)')
    plt.legend()
    ...
    plt.xlabel('Epochs')
    plt.ylabel('Metric')
    plt.title('Precision and Recall over Epochs (ResNet)')
    plt.legend()
